    ...
    while (date != end_date) {
        ...
        if (!yesterday_flag && prices.size() > 0) {
            ...
            if (prices.size() == 375) {
                ...
                Sim_To_CSV(Parent_Sim, date, string("_guide_") + underlying);
...
void Strategy_Simulator::Initialize_Solvers() {
    ...
    if (y_params.size() == config.thread_size) {
        std::cout << std::endl << "Importing params from " <<
                  config.Simulated_Parameter_Collection;
    }
