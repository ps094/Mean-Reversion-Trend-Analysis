...
Numerical_Sim_Results Numerical_Solver::Optimize() {
    ...
    float temp_dev = (PDiff.begin() + PDiff.size() / 4)->pos_diff + 0.001f;
    ...
    temp_dev = (MDiff.begin() + MDiff.size() / 5)->mov_diff + 0.001f;
    ...
    temp_dev = (PRDiff.begin() + PRDiff.size() / 5)->prof_diff + 0.001f;
